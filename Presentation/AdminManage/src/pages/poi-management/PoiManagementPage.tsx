...
export function PoiManagementPage() {
  ...
  const loadPois = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Không thể tải danh sách POIs",
      );
      setPois([]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="app-page">
        ...
        <div className="poi-filter">
          <div className="poi-filter-select">
            <svg viewBox="0 0 24 24" aria-hidden="true">
              <path d="M3 5h18l-7 8v6l-4 2v-8L3 5z" fill="currentColor" />
            </svg>
            <select
              className="app-input"
      ...
      {error ? (
        <div className="app-alert app-alert-danger">Lỗi: {error}</div>
      ) : null}
                ...
                <div className="poi-card-body">
                  ...
                  <div className="poi-card-meta">
                    ...
                    <div className="poi-card-coordinates">
                      {poi.position.lat.toFixed(4)},{" "}
                      {poi.position.lng.toFixed(4)}
                    </div>
                  </div>
                </div>
